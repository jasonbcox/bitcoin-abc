...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value,
    ...
    if (role == Qt::EditRole) {
        CTxDestination curAddress =
            DecodeDestination(rec->address.toStdString(), walletModel->getChainParams());
...
QString AddressTableModel::addRow(const QString &type, const QString &label,
    ...
    if (type == Send) {
        ...
        {
            if (walletModel->wallet().getAddress(DecodeDestination(strAddress, walletModel->getChainParams()))) {
                editStatus = DUPLICATE_ADDRESS;
                return QString();
            }
        }
