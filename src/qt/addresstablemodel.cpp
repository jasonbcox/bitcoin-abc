...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value,
    ...
    if (role == Qt::EditRole) {
        CTxDestination curAddress =
            DecodeDestination(rec->address.toStdString(), chainParams);
        if (index.column() == Label) {
            ...
            CTxDestination newAddress = DecodeDestination(
                value.toString().toStdString(), chainParams);
...
QString AddressTableModel::addRow(const QString &type, const QString &label,
    ...
    if (type == Send) {
        ...
        {
            if (walletModel->wallet().getAddress(DecodeDestination(strAddress, wallet->chainParams))) {
                editStatus = DUPLICATE_ADDRESS;
                return QString();
            }
        }
...
bool AddressTableModel::removeRows(int row, int count,
    ...
    walletModel->wallet().delAddressBook(
...
QString AddressTableModel::labelForAddress(const QString &address) const {
    {
        CTxDestination destination =
            DecodeDestination(address.toStdString(), chainParams);
        std::string name;
